...
export const ChatWidget: React.FC<ChatWidgetProps> = ({ studyId, studyGuide, sources = [], onUsageLimit }) => {
  ...
  if (!isOpen) return (<button onClick={() => setIsOpen(true)} className="fixed bottom-20 md:bottom-6 right-4 md:right-6 w-14 h-14 bg-indigo-600 text-white rounded-full shadow-lg hover:bg-indigo-700 transition-all flex items-center justify-center z-50 print:hidden animate-bounce-subtle" title="Falar com Professor Virtual"><MessageCircle className="w-8 h-8" /></button>);

  return (
    <div className="fixed bottom-20 md:bottom-6 right-4 md:right-6 w-[calc(100vw-2rem)] max-w-sm md:max-w-none md:w-96 h-[600px] max-h-[60vh] md:max-h-[85vh] bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col z-50 overflow-hidden font-sans print:hidden animate-in slide-in-from-bottom-10 fade-in duration-300">
