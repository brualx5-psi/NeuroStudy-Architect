...
export function App() {
  ...
  const [isParetoMode, setIsParetoMode] = useState(false); // Controls the simplified Pareto view
  ...
  const handleQuickStart = async (content: string | File, type: InputType, mode: StudyMode = StudyMode.NORMAL, autoGenerate: boolean = false) => {
    ...
    // Auto trigger generation for Pareto mode
    if (autoGenerate) {
  ...
  const handleParetoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file) {
          ...
          // Start process with SURVIVAL mode and auto-generate
  ...
  // Helper to generate without relying on activeStudyId state immediately
  ...
  const handleRegenerateGuide = async (newMode: StudyMode) => {
      ...
      // If we regenerate, we can stick to normal flow or keep pareto if we are there.
      // But usually user regenerates to see more detail, so we might want to exit Pareto mode if they choose Turbo/Normal.
      // For now, let's keep logic simple.
      handleGenerateGuideForStudy(activeStudy.id, source, newMode);
  };
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-800">
        <header className="px-8 py-6 flex justify-between items-center bg-white border-b border-gray-200">
            ...
            <button 
                onClick={() => { setView('app'); setIsParetoMode(false); }} 
  ...
  return (
    <div className="flex h-screen bg-white font-sans text-slate-800 overflow-hidden animate-in fade-in duration-500">
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        {/* Top Bar - HIDDEN IN PARETO MODE or Simplified */}
        ...
        {isParetoMode && activeStudy && (
            <header className="h-16 border-b border-red-100 bg-red-50 flex items-center justify-between px-8 flex-shrink-0 z-10">
                ...
                <div className="flex items-center gap-2">
                     <span className="text-xs text-red-600 uppercase font-bold tracking-wider">Modo Simplificado</span>
                </div>
            </header>
        )}
                ...
                <div className="max-w-5xl mx-auto">
                    ...
                    {activeTab === 'sources' && !isParetoMode && (
                        <div className="space-y-8 animate-fade-in">
                            ...
                            <div className={`border p-6 rounded-xl flex flex-col md:flex-row items-center justify-between gap-4 transition-colors ${
                                activeStudy.mode === StudyMode.SURVIVAL ? 'bg-red-50 border-red-100' : 
                                activeStudy.mode === StudyMode.TURBO ? 'bg-purple-50 border-purple-100' : 
                                'bg-indigo-50 border-indigo-100'
                            }`}>
                                <div>
                                    <h3 className={`font-bold text-lg ${
                                        activeStudy.mode === StudyMode.SURVIVAL ? 'text-red-900' : 
                                        activeStudy.mode === StudyMode.TURBO ? 'text-purple-900' : 
                                        'text-indigo-900'
                                    }`}>
                                        {activeStudy.mode === StudyMode.SURVIVAL ? 'Modo Pareto 80/20 (Foco Essencial)' : 
                                         activeStudy.mode === StudyMode.TURBO ? 'Modo Turbo (Detalhe Máximo)' : 
                                         'Pronto para transformar?'}
                                    </h3>
                                    <p className={`text-sm ${
                                        activeStudy.mode === StudyMode.SURVIVAL ? 'text-red-700' : 
                                        activeStudy.mode === StudyMode.TURBO ? 'text-purple-700' : 
                                        'text-indigo-700'
                                    }`}>
                                        {activeStudy.mode === StudyMode.SURVIVAL ? 'O NeuroStudy vai ignorar o ruído e extrair apenas os 20% do conteúdo que geram 80% do resultado.' : 
                                         activeStudy.mode === StudyMode.TURBO ? 'Análise granular e detalhada para quem não pode perder nenhuma informação.' : 
                                         'O NeuroStudy vai analisar suas fontes e criar o roteiro perfeito.'}
                                    </p>
                                </div>
                                <div className="flex items-center gap-4">
                                    <div className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-lg border border-gray-200">
                                        ...
                                        <div className="flex gap-1">
                                            <button 
                                                ...
                                                className={`p-1.5 rounded transition-colors ${activeStudy.mode === StudyMode.SURVIVAL ? 'bg-red-100 text-red-700 ring-2 ring-red-500 ring-offset-1' : 'hover:bg-gray-100 text-gray-400'}`}
                                                title="Pareto 80/20 (Essencial)"
                                            >
                                                <Target className="w-4 h-4"/>
                                            </button>
                                            <button 
                                                onClick={() => updateStudyMode(activeStudy.id, StudyMode.NORMAL)}
                                                className={`p-1.5 rounded transition-colors ${activeStudy.mode === StudyMode.NORMAL ? 'bg-blue-100 text-blue-700 ring-2 ring-blue-500 ring-offset-1' : 'hover:bg-gray-100 text-gray-400'}`}
                                            ...
                                            <button 
                                                ...
                                                className={`p-1.5 rounded transition-colors ${activeStudy.mode === StudyMode.TURBO ? 'bg-purple-100 text-purple-700 ring-2 ring-purple-500 ring-offset-1' : 'hover:bg-gray-100 text-gray-400'}`}
                                                title="Turbo (Completo)"
                                            >
                                    ...
                                    <button 
                                        ...
                                        className={`px-8 py-3 rounded-xl font-bold text-lg text-white shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 transition-transform active:scale-[0.99] ${
                                            activeStudy.mode === StudyMode.SURVIVAL ? 'bg-red-600 hover:bg-red-700 shadow-red-200' : 
                                            activeStudy.mode === StudyMode.TURBO ? 'bg-purple-600 hover:bg-purple-700 shadow-purple-200' : 
                                    ...
                                    >
                                        {processingState.isLoading ? (
                                            <>
                                                ...
                                                {processingState.step === 'transcribing' ? 'Transcrevendo...' :
                                                 processingState.step === 'analyzing' ? 'Lendo...' :
                                                 processingState.step === 'generating' ? 'Escrevendo...' : 
                                                 'Processando...'}
                                            </>
                                        ) : (
                                            <>
                                                {activeStudy.mode === StudyMode.SURVIVAL ? <Target className="w-5 h-5"/> : <BrainCircuit className="w-5 h-5" />} 
                                                {activeStudy.mode === StudyMode.SURVIVAL ? 'Extrair Pareto 80/20' : 'Gerar Roteiro'}
                                            </>
                                        )}
                                    </button>
                                </div>
                        ...
                        activeStudy.guide || processingState.isLoading ? (
                            processingState.isLoading && isParetoMode ? (
                                <div className="flex flex-col items-center justify-center h-96">
                                     ...
                                     <h3 className="text-xl font-bold text-gray-700">Extraindo a essência (80/20)...</h3>
                                ...
                                <ResultsView 
                                    ...
                                    onReset={() => {
                                        setActiveTab('sources');
                                        if(isParetoMode) setIsParetoMode(false); // Reset pareto if they hit reset
                                    }}
      ...
      {/* FLOATING CHAT - Hide on Pareto Mode to keep it clean, or keep it? User said "apenas um resumo". Let's hide it. */}
