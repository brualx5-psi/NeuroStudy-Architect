...
export function App() {
  ...
  const bookInputRef = useRef<HTMLInputElement>(null); // REF PARA INPUT DE LIVROS
  ...
  const createStudy = (folderId: string, title: string, mode: StudyMode = selectedMode, isBook: boolean = false) => {
    const newStudy: StudySession = {
      id: Date.now().toString(), folderId, title, sources: [], mode, isBook, // Flag de livro
  ...
  const addSourceToStudy = async () => { /* Same logic */ };
  const removeSource = (sourceId: string) => { /* Same logic */ };
  const handleStartRenamingSource = (source: StudySource) => { /* Same logic */ };
  const handleSaveSourceRename = () => { /* Same logic */ };
  ...
  const handleBookUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
      const file = e.target.files?.[0];
      if (file) {
          // Inicia como NORMAL por padrão, mas marca como isBook
          handleQuickStart(file, InputType.PDF, StudyMode.NORMAL, true, true); 
      }
  };
        ...
        <main className="flex-1 flex flex-col items-center justify-center p-6 text-center animate-fade-in">
            <div className="max-w-5xl mx-auto space-y-12">
                ...
                <div className="flex flex-col md:flex-row items-center justify-center gap-6">
                    ...
                    {/* 2. RESUMO DE LIVROS (LARANJA - NO MEIO) */}
                    <div className="relative group w-full md:w-80">
                        <input type="file" ref={bookInputRef} className="hidden" onChange={handleBookUpload} accept=".pdf"/>
          ...
          {activeStudy ? (
            <div className="max-w-5xl mx-auto space-y-6">
                ...
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
                                ...
                                <div className="space-y-4">
                                    {inputType === InputType.TEXT || inputType === InputType.DOI || inputType === InputType.URL ? (
                                        ...
                                        <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:bg-gray-50 transition-colors cursor-pointer relative">
                                            <input type="file" className="absolute inset-0 opacity-0 cursor-pointer" onChange={(e) => setSelectedFile(e.target.files?.[0] || null)} accept={inputType === InputType.PDF ? ".pdf" : inputType === InputType.VIDEO ? "video/*,audio/*" : "image/*"} />
                                            <div className="flex flex-col items-center gap-2 text-gray-500">
                                                {selectedFile ? (<><FileText className="w-8 h-8 text-indigo-500"/><span className="font-medium text-gray-900">{selectedFile.name}</span><span className="text-xs">Clique para trocar</span></>) : (<><UploadCloud className="w-8 h-8"/><span className="font-medium">Clique ou arraste o arquivo aqui</span><span className="text-xs">Suporta {inputType === InputType.PDF ? 'PDFs' : inputType === InputType.VIDEO ? 'Vídeo/Áudio' : 'Imagens (Cadernos/Lousas)'}</span></>)}
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    <div className="flex flex-col gap-4 justify-end pt-4 border-t border-gray-100 mt-4">
                                        <div className="flex items-center justify-end gap-2 text-sm text-gray-600">
                                            ...
                                            <select value={activeStudy.mode} onChange={(e) => updateStudyMode(activeStudy.id, e.target.value as StudyMode)} className="bg-white border border-gray-300 rounded px-2 py-1 text-sm font-medium focus:ring-2 focus:ring-indigo-500 outline-none">
                                                {/* CONDITIONAL OPTIONS FOR BOOK MODE */}
                                                {activeStudy.isBook ? (
                                                    <>
                                                        <option value={StudyMode.SURVIVAL}>Sobrevivência (Global)</option>
                                                        <option value={StudyMode.NORMAL}>Normal (Por Capítulo)</option>
                                                        <option value={StudyMode.HARD}>Hard (Detalhado)</option>
                                                    </>
                                                ) : (
                                                    <>
                    ...
                    {/* ... (Slides, Quiz, Flashcards views same as original) ... */}
