...
const getSourceText = (source: StudySourceInput) => {
  if (source.textContent) return source.textContent;
  if (source.type === 'PDF') return extractTextFromPdfBase64(source.content || '');
  if (source.type === 'EPUB' || source.type === 'MOBI') return estimateTextFromBinary(source.content || '');
